# TemplateLab Backend API

Node.js/Express backend for the TemplateLab Template & Pipeline Builder Platform.
...
```bash
psql -U postgres -d templatelab -f database/schema.sql
```
...
```bash
npm install -g pm2
pm2 start dist/server.js --name templatelab-api
```
